...
class LoginController extends Controller
{
    ...
    public function doLogin(Request $request)
    {
		...
		if(!$check_email){
            ...
            if (!$check_email || !Hash::check($request->user_password, $user->user_password)) {
                return redirect("login")->withErrors(['user_login' => ['Email atau sandi yang Anda masukkan salah']]);
            }
            else {
                if($user->user_status != 2){
                    return redirect("login")->withErrors(['user_login' => ['Status akun anda sudah tidak aktif, silahkan menghubungi tim IT Kantor Pusat jika anda ingin mengaktifkan akun anda kembali']]);
                }
                // create token data
