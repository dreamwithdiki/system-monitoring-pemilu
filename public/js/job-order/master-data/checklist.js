...
$(function () {
  ...
  if (ac_checklist_group.length) {
    ...
    $this.wrap('<div class="position-relative"></div>').select2({
      ...
      ajax: {
        url: baseUrl + 'autocomplete/job-checklist-group/find',
        dataType: 'json',
  ...
  if (dt_ajax_table.length) {
    var dt_ajax = dt_ajax_table.DataTable({
      ...
      initComplete: onInit,
      ajax: {
        url: baseUrl + 'job-order/master-data/checklist/get',
        beforeSend: function () {
          window.Helpers.blockUIPageLoader(baseUrl + 'job-order/master-data/checklist/get');
        },
        complete: function () {
          $.unblockUI();
        },
      ...
      columns: [
        ...
        { data: 'job_checklist_name' },
        { data: 'job_checklist_group_name' },
        { data: 'job_checklist_reference' },
        {
          data: 'job_checklist_desc',
          orderable: false,
        ...
        { data: 'job_checklist_status', orderable: false },
      ],
      columnDefs: [
        {
          targets: 5,
        ...
        },
        {
          ...
          render: function (data, type, row, meta) {
            return '' +
              ...
              '<a id="dropdownMenuEdit" data-id="' + row.job_checklist_id + '" class="dropdown-item" href="javascript:void(0);"><i class="bx bx-edit me-1"></i> Edit</a>' +
              '<a id="dropdownMenuActivate" data-id="' + row.job_checklist_id + '" data-status="2" class="dropdown-item dropdownMenuStatusUpdate" href="javascript:void(0);"><i class="bx bx-check me-1"></i> Activate</a>' +
              '<a id="dropdownMenuDeactivate" data-id="' + row.job_checklist_id + '" data-status="1" class="dropdown-item dropdownMenuStatusUpdate" href="javascript:void(0);"><i class="bx bx-x me-1"></i> Deactivate</a>' +
              '<a id="dropdownMenuDelete" data-id="' + row.job_checklist_id + '" class="dropdown-item" href="javascript:void(0);"><i class="bx bx-trash me-1"></i> Delete</a>' +
              '</div>' +
              '</div>';
          }
        }
  ...
  var fv = FormValidation.formValidation(add_checklist_form, {
    fields: {
      ...
      job_checklist_reference: {
        validators: {
          notEmpty: {
            message: 'Please enter checklist reference'
          }
        }
      },
      job_checklist_sort: {
        validators: {
          notEmpty: {
            message: 'Please enter checklist sort'
          }
        }
      },
      job_checklist_desc: {
    ...
    if ($('#formAddChecklist').data('method') == 'add') {
      var url = "job-order/master-data/checklist/store";
    } else if ($('#formAddChecklist').data('method') == 'edit') {
      var url = "job-order/master-data/checklist/update/" + $('#formAddChecklist').attr('data-id');
    } else {
      var url = "";
    }
  ...
  $(document).on('click', '#dropdownMenuEdit', function () {
    ...
    // get data
    $.ajax({
      url: baseUrl + "job-order/master-data/checklist/show/" + checklist_id,
      type: 'GET',
      beforeSend: function (data) {
        window.Helpers.blockUIModalLoader(modal_class_loader);
      },
      success: function (response) {
        if (response.data.job_checklist_status == 2) {
        ...
        if (response.data.checklist_group) {
          var option = new Option(response.data.checklist_group.job_checklist_group_name, response.data.checklist_group.job_checklist_group_id, true, true);
          $('#addChecklistGroup').append(option).trigger('change');
        }

        $('#addName').val(response.data.job_checklist_name);

        if (response.data.job_checklist_is_freetext == 2) {
          $('#addChecklistIsFreetext').prop('checked', true);
        } else {
          $('#addChecklistIsFreetext').prop('checked', false);
        }

        $('#addSort').val(response.data.job_checklist_sort);
        $('#addReference').val(response.data.job_checklist_reference);
        $('#addDesc').val(response.data.job_checklist_desc);
  ...
  $(document).on('click', '.dropdownMenuStatusUpdate', function () {
    ...
    Swal.fire({
      ...
      if (result.value) {
        $.ajax({
          data: { job_checklist_id: checklist_id, job_checklist_status: checklist_status },
          type: 'POST',
          url: baseUrl + 'job-order/master-data/checklist/update-status',
          success: function success(response) {
            dt_ajax.draw();
            Swal.fire({
              icon: 'success',
  ...
  $(document).on('click', '#dropdownMenuDelete', function () {
    ...
    Swal.fire({
      ...
      if (result.value) {
        $.ajax({
          data: { job_checklist_id: checklist_id },
          type: 'POST',
          url: baseUrl + 'job-order/master-data/checklist/delete',
          success: function success(response) {
            dt_ajax.draw();
            Swal.fire({
              icon: 'success',
