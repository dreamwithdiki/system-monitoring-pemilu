...
$(function () {
  ...
  if (dt_ajax_table.length) {
    var dt_ajax = dt_ajax_table.DataTable({
      ...
      initComplete: onInit,
      ajax: {
        url: baseUrl + 'site-visit/master-data/checklist-group/get',
        beforeSend: function () {
          window.Helpers.blockUIPageLoader(baseUrl + 'site-visit/master-data/checklist-group/get');
        },
        complete: function () {
          $.unblockUI();
        },
      ...
      columns: [
        ...
        { data: 'checklist_group_status', orderable: false },
      ],
      columnDefs: [
        {
          targets: 5,
          ...
          render: function (data, type, row, meta) {
              ...
              '<div class="d-inline-block text-nowrap">' +
                  ...
                  '<div class="dropdown-menu">' +
                      '<a id="dropdownMenuEdit" data-id="' + row.checklist_group_id + '" class="dropdown-item" href="javascript:void(0);"><i class="bx bx-edit me-1"></i> Edit</a>' +
                      '<a id="dropdownMenuActivate" data-id="' + row.checklist_group_id + '" data-status="2" class="dropdown-item dropdownMenuStatusUpdate" href="javascript:void(0);"><i class="bx bx-check me-1"></i> Activate</a>' +
                      '<a id="dropdownMenuDeactivate" data-id="' + row.checklist_group_id + '" data-status="1" class="dropdown-item dropdownMenuStatusUpdate" href="javascript:void(0);"><i class="bx bx-x me-1"></i> Deactivate</a>' +
                      '<a id="dropdownMenuDelete" data-id="' + row.checklist_group_id + '" class="dropdown-item" href="javascript:void(0);"><i class="bx bx-trash me-1"></i> Delete</a>' +
                  '</div>' +
              '</div>';
          }
        }
  ...
  var fv = FormValidation.formValidation(add_checklist_group_form, {
    fields: {
      checklist_group_sort: {
        validators: {
          notEmpty: {
            message: 'Please enter checklist group sort'
          }
        }
      },
        checklist_group_code: {
        validators: {
          notEmpty: {
    ...
    if ($('#formAddChecklistGroup').data('method') == 'add') {
      var url = "site-visit/master-data/checklist-group/store";
    } else if ($('#formAddChecklistGroup').data('method') == 'edit') {
      var url = "site-visit/master-data/checklist-group/update/" + $('#formAddChecklistGroup').attr('data-id');
    } else {
      var url = "";
    }
  ...
  $(document).on('click', '#dropdownMenuEdit', function () {
    ...
    // get data
    $.ajax({
      url: baseUrl + "site-visit/master-data/checklist-group/show/" + checklist_group_id,
      type: 'GET',
      beforeSend: function(data) {
        window.Helpers.blockUIModalLoader(modal_class_loader);
      },
      success: function(response) {
        if (response.data.checklist_group_status == 2) {
          $('#addStatus').prop('checked', true);
        } else {
          $('#addStatus').prop('checked', false);
        }

        $('#addSort').val(response.data.checklist_group_sort);
        $('#addCode').val(response.data.checklist_group_code);
        $('#addName').val(response.data.checklist_group_name);
        $('#addDesc').val(response.data.checklist_group_desc);
  ...
  $(document).on('click', '.dropdownMenuStatusUpdate', function () {
    ...
    Swal.fire({
      ...
      if (result.value) {
        $.ajax({
          data: {checklist_group_id: checklist_group_id, checklist_group_status: checklist_group_status},
          type: 'POST',
          url: baseUrl + 'site-visit/master-data/checklist-group/update-status',
          success: function success(response) {
            dt_ajax.draw();
            Swal.fire({
              icon: 'success',
  ...
  $(document).on('click', '#dropdownMenuDelete', function () {
    ...
    Swal.fire({
      ...
      if (result.value) {
        $.ajax({
          data: {checklist_group_id: checklist_group_id},
          type: 'POST',
          url: baseUrl + 'site-visit/master-data/checklist-group/delete',
          success: function success(response) {
            dt_ajax.draw();
            Swal.fire({
              icon: 'success',
