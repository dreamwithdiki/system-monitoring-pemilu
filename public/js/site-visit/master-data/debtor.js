...
$(function () {
  ...
  var modal_add_debtor = $('#modalAddDebtor');
  ...
  $.ajax({
    url: baseUrl + 'site-visit/master-data/debtor/get-provinces',
    type: 'GET',
    dataType: 'json',
  ...
  $('#addProvince').on('change', function(){
    ...
    $.ajax({
      url: baseUrl + 'site-visit/master-data/debtor/get-regencies',
      type: 'GET',
      ...
      success: function(response) {
        if (response.status) {
          ...
          // $('#addRegency').append('<option value="">' + response.data + '</option>');
          // $('#addDistrict').append('<option value="">' + response.data + '</option>');
  ...
  $('#addRegency').on('change', function() {
    ...
    $.ajax({
      url: baseUrl + 'site-visit/master-data/debtor/get-districts',
      type: 'GET',
  ...
  $('#addDistrict').on('change', function() {
    ...
    $.ajax({
      url: baseUrl + 'site-visit/master-data/debtor/get-villages',
      type: 'GET',
  ...
  if (dt_ajax_table.length) {
    var dt_ajax = dt_ajax_table.DataTable({
      ...
      initComplete: onInit,
      ajax: {
        url: baseUrl + 'site-visit/master-data/debtor/get',
        beforeSend: function () {
          window.Helpers.blockUIPageLoader(baseUrl + 'site-visit/master-data/debtor/get');
        },
        complete: function () {
          $.unblockUI();
        },
      ...
      columns: [
        ...
        { data: 'debtor_status', orderable: false }
      ],
      columnDefs: [
        {
          targets: 3,
          ...
          render: function (data, type, row, meta) {
              ...
              '<div class="d-inline-block text-nowrap">' +
                  ...
                  '<div class="dropdown-menu">' +
                      '<a id="dropdownMenuEdit" data-id="' + row.debtor_id + '" class="dropdown-item" href="javascript:void(0);"><i class="bx bx-edit me-1"></i> Edit</a>' +
                      '<a id="dropdownMenuActivate" data-id="' + row.debtor_id + '" data-status="2" class="dropdown-item dropdownMenuStatusUpdate" href="javascript:void(0);"><i class="bx bx-check me-1"></i> Activate</a>' +
                      '<a id="dropdownMenuDeactivate" data-id="' + row.debtor_id + '" data-status="1" class="dropdown-item dropdownMenuStatusUpdate" href="javascript:void(0);"><i class="bx bx-x me-1"></i> Deactivate</a>' +
                      '<a id="dropdownMenuDelete" data-id="' + row.debtor_id + '" class="dropdown-item" href="javascript:void(0);"><i class="bx bx-trash me-1"></i> Delete</a>' +
                      '<div class="dropdown-divider"></div>' +
                      '<a id="dropdownMenuContact" data-id="' + row.debtor_id + '" data-name="' + row.debtor_name + '" class="dropdown-item" href="javascript:void(0);"><i class="bx bx-id-card me-1"></i> Contact</a>' +
                  '</div>' +
              '</div>';
          }
        }
  ...
  // Add Form
  var add_debtor_form = document.getElementById('formAddDebtor');

  // Debtor Form Validation
  var fv = FormValidation.formValidation(add_debtor_form, {
    fields: {
      debtor_name: {
        validators: {
          notEmpty: {
    ...
    // Adding or Updating debtor when form successfully validate
    if ($('#formAddDebtor').data('method') == 'add') {
      var url = "site-visit/master-data/debtor/store";
    } else if ($('#formAddDebtor').data('method') == 'edit') {
      var url = "site-visit/master-data/debtor/update/" + $('#formAddDebtor').attr('data-id');
    } else {
      var url = "";
    }

    $.ajax({
      data: $('#formAddDebtor').serialize(),
      url: baseUrl + url,
      type: 'POST',
      success: function success(response) {
        dt_ajax.draw();
        modal_add_debtor.modal('hide');
  ...
  $(document).on('click', '#dropdownMenuEdit', function () {
    ...
    // get data
    $.ajax({
      url: baseUrl + "site-visit/master-data/debtor/show/" + debtor_id,
      type: 'GET',
      ...
      success: function(response) {
        ...
        if (response.data.debtor_status == 2) {
          $('#addStatus').prop('checked', true);
        } else {
          $('#addStatus').prop('checked', false);
        }

        $('#addName').val(response.data.debtor_name);
        $('#addPhone').val(response.data.debtor_phone);
        $('#addFax').val(response.data.debtor_fax);
        $('#addEmail').val(response.data.debtor_email);
        $('#addPostalCode').val(response.data.debtor_postal_code);
        ...
        $('#addAddress').val(response.data.debtor_address);

        $('#addDesc').val(response.data.debtor_desc);
        modal_class_loader.unblock();
      }
    });

    $('#addFormLabel > p').html('Edit debtor.');
    $('#formAddDebtor').attr('data-method', 'edit');
    $('#formAddDebtor').data('method', 'edit');
    $('#formAddDebtor').attr('data-id', debtor_id);
    modal_add_debtor.modal('show');
  });
  
  // Active / Deactive status button handler
  $(document).on('click', '.dropdownMenuStatusUpdate', function () {
    var debtor_id = $(this).data('id'),
      debtor_status = $(this).data('status');
    ...
    Swal.fire({
      ...
      if (result.value) {
        $.ajax({
          data: {debtor_id: debtor_id, debtor_status: debtor_status},
          type: 'POST',
          url: baseUrl + 'site-visit/master-data/debtor/update-status',
          success: function success(response) {
            dt_ajax.draw();
            Swal.fire({
              icon: 'success',
    ...
    Swal.fire({
      ...
      if (result.value) {
        $.ajax({
          ...
          type: 'POST',
          url: baseUrl + 'site-visit/master-data/debtor/delete',
          success: function success(response) {
            dt_ajax.draw();
            Swal.fire({
              icon: 'success',
  ...
  modal_add_debtor.on('hidden.bs.modal', function () {
    $('#addFormLabel > p').html('Add new debtor.');
    $('#formAddDebtor').attr('data-method', 'add');
    $('#formAddDebtor').data('method', 'add');
  ...
  if (dt_ajax_table_contact_c.length) {
    var dt_ajax_table_contact = dt_ajax_table_contact_c.DataTable({
      ...
      serverSide: true,
      ajax: {
        url: baseUrl + 'site-visit/master-data/debtor/contact/get',
        data: function(d) {
          d.debtor_id = debtor_id_param;
        }
      },
      columns: [
          {data: 'no', orderable: false }, 
          {data: 'debtor_contact_fullname'},
          {data: 'debtor_contact_email'},
          {data: 'debtor_contact_mobile_phone'},
          {data: 'debtor_contact_phone'},
          {data: 'debtor_contact_status', orderable: false },
          {data: 'debtor_contact_actions', orderable: false },
      ],
      columnDefs: [
        {
          targets: 5,
        ...
        },
        {
          ...
          render: function (data, type, row, meta) {
              ...
              '<div class="d-inline-block text-nowrap">' +
                  '<button type="button" class="btn p-0 dropdown-toggle hide-arrow" data-bs-toggle="dropdown"><i class="bx bx-dots-vertical-rounded"></i></button>' +
                  '<div class="dropdown-menu">' +
                      '<a id="dropdownMenuEditContact" data-id="' + row.debtor_contact_id + '" class="dropdown-item" href="javascript:void(0);"><i class="bx bx-edit-alt me-1"></i> Edit</a>' +
                      '<a id="dropdownMenuActivateContact" data-id="' + row.debtor_contact_id + '" data-status="2" class="dropdown-item dropdownMenuStatusUpdateContact" href="javascript:void(0);"><i class="bx bx-check me-1"></i> Activate</a>' +
                      '<a id="dropdownMenuDeactivateContact" data-id="' + row.debtor_contact_id + '" data-status="1" class="dropdown-item dropdownMenuStatusUpdateContact" href="javascript:void(0);"><i class="bx bx-x me-1"></i> Deactivate</a>' +
                      // '<a id="dropdownMenuDeleteContact" data-id="' + row.debtor_contact_id + '" class="dropdown-item" href="javascript:void(0);"><i class="bx bx-trash me-1"></i> Delete</a>' +
                  '</div>' +
              '</div>';
          }
        }
  ...
  $(document).on('click', '#dropdownMenuContact', function () {
      var debtor_id = $(this).data('id');
      var debtor_name = $(this).data('name');
  
      // Reinit contact with new param debtor_id
      debtor_id_param = debtor_id
      dt_ajax_table_contact.draw();

      $('#addFormContactlabel > h3').html('<b>'+debtor_name+'</b>' + ' - Contact');
    ...
    var fv_add_contact = FormValidation.formValidation(addNewContactForm, {
      ...
      var url = "site-visit/master-data/debtor/contact/store";

      $.ajax({
        data: form_contact.serialize() + "&debtor_id=" + debtor_id_param,
        url: baseUrl + url,
        type: 'POST',
        success: function success(response) {
          dt_ajax_table_contact.draw();
  ...
  $(document).on('click', '#dropdownMenuEditContact', function () {
    var debtor_contact_id = $(this).data('id');

    // get data
    $.get("".concat(baseUrl, "site-visit/master-data/debtor/contact/show/").concat(debtor_contact_id), function (response) {
      if (response.data.debtor_contact_status == 2) {
        $('#editContactStatus').prop('checked', true);
      } else {
        $('#editContactStatus').prop('checked', false);
      }
      $('#editContactFullname').val(response.data.debtor_contact_fullname);
      $('#editContactEmail').val(response.data.debtor_contact_email);
      $('#editContactMobilePhone').val(response.data.debtor_contact_mobile_phone);
      $('#editContactPhone').val(response.data.debtor_contact_phone);
    });
    
    $('#formEditContact').attr('data-method', 'edit');
    $('#formEditContact').data('method', 'edit');
    $('#formEditContact').data('id', debtor_contact_id);
    $('#formEditContact').attr('data-id', debtor_contact_id);
  ...
  var fv_edit_contact = FormValidation.formValidation(editContactForm, {
    ...
    var url = "site-visit/master-data/debtor/contact/update/" + form_contact.attr('data-id');
  ...
  $(document).on('click', '.dropdownMenuStatusUpdateContact', function () {
    var debtor_contact_id = $(this).data('id'),
      debtor_contact_status = $(this).data('status');

    if (debtor_contact_status == 2) {
      var confirmText = 'Yes, active it!',
        confirmStyle = 'btn btn-success me-3';
    } else {
      var confirmText = 'Yes, deactive it!',
    ...
    Swal.fire({
      ...
      if (result.value) {
        $.ajax({
          data: {debtor_contact_id: debtor_contact_id, debtor_contact_status: debtor_contact_status},
          type: 'POST',
          url: baseUrl + 'site-visit/master-data/debtor/contact/update-status',
          success: function success(response) {
            dt_ajax_table_contact.draw();
            Swal.fire({
              icon: 'success',
  ...
  $(document).on('click', '#dropdownMenuDeleteContact', function () {
    ...
    Swal.fire({
      ...
      if (result.value) {
        $.ajax({
          data: {debtor_contact_id: debtor_contact_id},
          type: 'POST',
          url: baseUrl + 'site-visit/master-data/debtor/contact/delete',
          success: function success(response) {
            dt_ajax_table_contact.draw();
            Swal.fire({
              icon: 'success',
